package com.ant.mclangsplit.forge.config;
...
import com.ant.mclangsplit.common.config.ConfigHelper;
...
public class ClothConfig {
    ...
    public static ConfigBuilder getBuilder() {
        ...
        ConfigBuilder builder = ConfigBuilder.create()
                .setTitle(Component.translatable("config." + MCLangSplit.MOD_NAME + ".title"))
        ...
        defaultCategory.addEntry(entryBuilder.startTextDescription(Component.translatable("config." + MCLangSplit.MOD_NAME + ".description")).build());
            ...
            if (classes.stream().anyMatch(c -> c.isAssignableFrom(field.getType()) && !c.isEnum())) {
                ...
                if (configCategoryField.isPresent()) {
                    try {
                        ...
                        ConfigCategory category = builder.getOrCreateCategory(Component.translatable("config." + MCLangSplit.MOD_NAME + ".category." + categoryString));
    ...
    private static AbstractConfigListEntry<?> fieldToEntry(ConfigEntryBuilder entryBuilder, Field field, Object obj, String category) {
        try {
            Component fieldComponent = Component.translatable("config." + MCLangSplit.MOD_NAME + ".option." + category + "." + field.getName());
